...
the semiparametric `nomad = TRUE` route now sits on the same modern search
...
For current LP-capable Ichimura routes, the underlying objective evaluation has
also been tightened substantially. In practice, that means the continuous
single-index route is now a much more plausible first stop when you want
dimension reduction together with the newer local-polynomial search machinery.
