...
::: {.callout-note}
## Plot interface update in `np` / `npRmpi` `0.70-2`

The plot interface was redesigned in `0.70-2`. Current gallery examples now use the new public controls such as `errors`, `bootstrap`, `band`, `data_rug`, `data_overlay`, and other `snake_case` option names. Older plotting code may need small argument-name updates.
:::
...
- [Conditional mode classification](classification_modes.qmd): `npconmode` now has a direct formula/data workflow, propagates conditional-density search controls including `nomad = TRUE`, mirrors selected degree/search metadata in returned objects and summaries, can properize fitted class probabilities for non-local-constant routes before selecting the modal outcome, and supports level-specific class-probability effects.
