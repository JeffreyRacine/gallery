...
- [Variability bounds on plots](regression.qmd): Bonferroni and simultaneous bounds are new additions to the plotting surface, while `plot.errors.type = "all"` now presents those together with the traditional pointwise bounds.
- [Interactive plotting surfaces](plotting_and_intervals.qmd): `renderer = "rgl"` now supports interactive 3D rotation/zoom when `rgl` is installed, `plot.rug = TRUE` adds 1D/2D rugs, and `plot.data.overlay = TRUE/FALSE` controls whether the data are shown directly on y/x plots.
