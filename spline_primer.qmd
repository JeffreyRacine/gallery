...
In current `crs`, the helper `crshat()` builds the fitted-function or derivative
hat operator directly. That keeps constrained spline scripts close in spirit to
the `npreghat()` examples in `np` and avoids older manual basis-matrix
construction in most workflows.
...
- [`crs` getting started vignette on CRAN](https://cran.r-project.org/web/packages/crs/vignettes/crs_getting_started.html)
